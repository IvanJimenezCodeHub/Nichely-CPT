...
class PostFormPage extends React.Component {
  ...
  GMap(){
    return (
        ...
        <Marker position={{lat: geocodeByAddress(this.eventLocation).then(results => getLatLng(results[0].geometry.location.lat)), lng: geocodeByAddress(eventLocation).then(results => getLatLng(results[0].geometry.location.lng))}}></Marker>
        </GoogleMap>
    );
  }
