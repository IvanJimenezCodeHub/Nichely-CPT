  ...
  GMap(){
    return (
        ...
        {this.state.events.map(event =>(
          <Marker 
            key = {event.id}
            position={{lat: geocodeByAddress(this.state.eventLocation).then(results => getLatLng(results[0].geometry.location.lat)), lng: geocodeByAddress(this.state.eventLocation).then(results => getLatLng(results[0].geometry.location.lng))}}></Marker>
        ))}
